...
3. Head-to-Head Matchup Win Rate & Counter Efficiency (Empirical AR-035 Results)
...
values_v5 = [94, 98, 95, 92, 100, 98, 96, 98] # Apex Omnipotent v5 + AR-035 Tech
...
# Plot Radar lines
ax1.plot(angles, values_v5, color='#10b981', linewidth=2.8, linestyle='solid', label='Apex Decks (001/004/005/006)', zorder=5)
...
agents_list = ['Apex 006 Omni', '001 v5 Omnipotent', 'lb1009 (Lucario)', 'lb945 (Multiply)', 'lb826 (Alakazam)', 'lb814 (Crustle)', 'Stage 4 FP32', 'Starter #251']
elo_invar = [1092.0, 1065.0, 1009.0, 945.0, 826.0, 814.0, 680.0, 450.0]
...
# -------------------------------------------------------------
# 3. BOTTOM-LEFT: Head-to-Head Matchup Matrix & Empirical Results
# -------------------------------------------------------------
...
wr_ar034 = [0.0, 0.0, 25.0, 75.0, 80.0, 95.0] # AR-034
wr_ar035 = [25.0, 0.0, 25.0, 75.0, 85.0, 100.0] # AR-035 Empirical (v5 took 1st Lucario win!)
wr_target = [75.0, 75.0, 50.0, 75.0, 90.0, 100.0] # Next Goal

x_pos = np.arange(len(matchups))
w = 0.26

b1 = ax3.bar(x_pos - w, wr_ar034, width=w, label='AR-034 Panel', color='#475569', edgecolor='#1e293b')
b2 = ax3.bar(x_pos, wr_ar035, width=w, label='AR-035 Empirical (1st Win on Lucario!)', color='#10b981', edgecolor='#1e293b')
b3 = ax3.bar(x_pos + w, wr_target, width=w, label='AR-036 Projected Target', color='#06b6d4', edgecolor='#1e293b')
...
ax3.set_title('⚔️ EMPIRICAL HEAD-TO-HEAD TOURNAMENT EVOLUTION (AR-034 vs AR-035)', fontsize=12.5, fontweight='bold', color='#38bdf8', pad=12)
